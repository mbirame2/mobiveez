...
class EmarketController extends Controller
{
    ...
    {
      $annonce =annonce::with('departement')->where('idannonce',$id)->select('titre','prix','bloquer_commande','statut','localisation','typelivraison','typelivraison','id_dep','idannonce','referenceannonce','idmembre','idsouscategorie','description','nomvendeur','paiementtranche','typeannonce','dateannonce','validite')->first();   
      if($annonce){
  ...
  public function updateannonce(Request $req, ApiController $apicontroller){
    ...
    //  array_push($details, $immobilier);
      array_push($details, $annonce);
          ...
          if($req->input('designation_modelevoiture')){
            ...
            $modele->idmarquevoiture=$req->input('idmarquevoiture') || $req->input('idmarquemoto');
            $modele->save();
    ...
    if( !annoncesboutique::where([['idannonce', $annonce['idannonce']],['idshowroom',$req['idshowroom']]])->exists() && $req->input('idshowroom')){
      $iman= new annoncesboutique;
...
// Rejeter une offre
    ...
    {
      $ann=annonce::where('statut','acceptee')->select('idannonce')->get();
